...
static void coa_request(struct radius_pd_t *rpd)
{
	...
	if (ev.res)
		...
		/*attr = rad_packet_find_attr(rpd->dm_coa_req, NULL, "MicroBNG-Bandwidth-Max-Up");
		if (attr){
			int microbng_bandwidth_max_up = attr->val.integer;
			attr = rad_packet_find_attr(rpd->dm_coa_req, NULL, "MicroBNG-Bandwidth-Max-Down");
			if (attr){
				int microbng_bandwidth_max_down = attr->val.integer;
				rpd->ses->microbng_bandwidth_max_up = microbng_bandwidth_max_up;
				rpd->ses->microbng_bandwidth_max_down = microbng_bandwidth_max_down;
				vpp_api_policer_update(rpd->ses->vpp_policer_id_up, microbng_bandwidth_max_up, microbng_bandwidth_max_up * 100, 0, 0);
				vpp_api_policer_update(rpd->ses->vpp_policer_id_down, microbng_bandwidth_max_down, microbng_bandwidth_max_down * 100, 0, 0);
				send_ack = 1;
				goto out;
			}
		}*/
...
#ifdef HAVE_VRF
		attr = rad_packet_find_attr(rpd->dm_coa_req, "Micro-BNG", "Bng-VRF-Name");
		if (attr){
