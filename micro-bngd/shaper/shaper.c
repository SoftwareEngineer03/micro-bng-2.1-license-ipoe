...
static int check_radius_attrs(struct shaper_pd_t *pd, struct rad_packet_t *pack)
{
  ...
  pd->ses->microbng_bandwidth_max_down = dflt_down_speed;
  pd->ses->microbng_bandwidth_max_up = dflt_up_speed;
	...
	list_for_each_entry(attr, &pack->attrs, entry) {
		...
		if (down_speed)
      pd->ses->microbng_bandwidth_max_down = down_speed;
			//tr_pd->down_speed = down_speed;
		if (down_burst)
			tr_pd->down_burst = down_burst;
		if (up_speed)
      pd->ses->microbng_bandwidth_max_up = up_speed;
...
static void ev_radius_access_accept(struct ev_radius_t *ev)
{
  ...
  log_ppp_debug(" ### ACCESS_ACCEPT max_up: %d, max_down: %d, min_up: %d, min_down: %d\n", ev->ses->microbng_bandwidth_max_up,
      ev->ses->microbng_bandwidth_max_down, ev->ses->microbng_bandwidth_min_up,
      ev->ses->microbng_bandwidth_min_down);
}
...
static void ev_radius_coa(struct ev_radius_t *ev)
{
  ...
  log_ppp_debug(" ### COA max_up: %d, max_down: %d, min_up: %d, min_down: %d\n", ev->ses->microbng_bandwidth_max_up,
      ev->ses->microbng_bandwidth_max_down, ev->ses->microbng_bandwidth_min_up,
      ev->ses->microbng_bandwidth_min_down);

  vpp_api_policer_update(ev->ses->vpp_policer_id_up, ev->ses->microbng_bandwidth_max_up, ev->ses->microbng_bandwidth_max_up * 100, 0, 0);
  vpp_api_policer_update(ev->ses->vpp_policer_id_down, ev->ses->microbng_bandwidth_max_down, ev->ses->microbng_bandwidth_max_down * 100, 0, 0);
...
static void ev_shaper(struct ev_shaper_t *ev)
{
  ...
  ev->ses->microbng_bandwidth_max_down = down_speed;
  ev->ses->microbng_bandwidth_max_up = up_speed;

  log_ppp_debug(" ### EV_SHAPER max_up: %d, max_down: %d, min_up: %d, min_down: %d\n", ev->ses->microbng_bandwidth_max_up,
        ev->ses->microbng_bandwidth_max_down, ev->ses->microbng_bandwidth_min_up,
        ev->ses->microbng_bandwidth_min_down);
