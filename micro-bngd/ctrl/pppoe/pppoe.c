...
struct pppoe_conn_t {
  ...
  unsigned int session_started:2;
...
struct pppoe_interface_config_t conf_interface_config[MAX_CONF_INTERFACES];
obj_hash_table_t conf_interface_pado_delay;
obj_hash_table_t *ptr_conf_interface_pado_delay = NULL;
...
unsigned int stat_session_active;
...
static inline int pado_iface_cmpfunc_obj(const void *const a, const void *const b, const int size)
{
...
static void disconnect(struct pppoe_conn_t *conn)
{
  ...
  uint64_t key64;
  key64 = *((uint64_t *) (conn->addr)) << 16;
  key64 = (key64 & ~0xffff) | (uint64_t) (conn->sid);
  hashtable64_ts_free(&serv->mac_sessid_2_conn, key64);

	if (conn->ppp_started) {
		dpado_check_prev(__sync_fetch_and_sub(&stat_active, 1), conn->ctrl.ifname);
		conn->ppp_started = 0;
		ap_session_terminate(&conn->ppp.ses, TERM_USER_REQUEST, 1);
	}

  if(conn->session_started == 1) {
    __sync_sub_and_fetch(&stat_session_active, 1);
    conn->session_started = 2;
  } else if(conn->session_started == 0) {
...
static void ppp_started(struct ap_session *ses)
{
  ...
  conn->session_started = 1;
  
  __sync_fetch_and_sub(&stat_starting, 1);
  __sync_add_and_fetch(&stat_session_active, 1);
}
...
static struct pppoe_conn_t *allocate_channel(struct pppoe_serv_t *serv, const uint8_t *addr, const struct pppoe_tag *host_uniq, const struct pppoe_tag *relay_sid, const struct pppoe_tag *service_name, const struct pppoe_tag *tr101, const uint8_t *cookie, uint16_t ppp_max_payload)
{
    ...
    //memcpy(conn->addr, serv->hwaddr, ETH_ALEN);
    /*log_debug(" *** %s:%02X:%02X:%02X:%02X:%02X:%02X\n", serv->ifname,
  ...
  conn->ppp.is_processing = 0;
  pthread_mutex_init(&conn->ppp.lock, NULL);

  conn->ppp.disc_sock = serv->disc_sock;
  conn->ppp.ppp_sock = serv->ppp_sock;
  conn->ppp.ifindex = serv->ifindex;

  memcpy(conn->ppp.hwaddr, conn->serv->hwaddr, ETH_ALEN);
  memcpy(conn->ppp.addr, conn->addr, ETH_ALEN); // ppp client mac
  //conn->serv->hwaddr, conn->addr

	conn->ppp.ses.net = serv->net;
	conn->ppp.ses.ctrl = &conn->ctrl;
	conn->ppp.ses.chan_name = conn->ctrl.calling_station_id;

  if( strstr(serv->ifname, "-c") && !strncmp(serv->ifname, "vlan", 4) ) {
    int parent_vid = atoi(serv->ifname+4);
    conn->ppp.vid = parent_vid*10000 + serv->vid;
  } else {
	  conn->ppp.vid = serv->vid;
  }
  ...
  uint8_t *keyCookie;
  keyCookie = conn->cookie;
  obj_hashtable_ts_insert(&serv->cookie_2_conn, keyCookie, COOKIE_LENGTH - 4, conn);

  uint64_t key64;
  key64 = *((uint64_t *) (conn->addr)) << 16;
  key64 = (key64 & ~0xffff) | (uint64_t) (conn->sid);
  hashtable64_ts_insert(&serv->mac_sessid_2_conn, key64, conn);
...
static void connect_channel(struct pppoe_conn_t *conn)
{
  ...
  __sync_add_and_fetch(&stat_starting, 1);
	...
	dpado_check_next(__sync_add_and_fetch(&stat_active, 1), conn->ctrl.ifname);
...
static void pppoe_send_PADO(struct pppoe_serv_t *serv, const uint8_t *addr, const struct pppoe_tag *host_uniq, const struct pppoe_tag *relay_sid, const struct pppoe_tag *service_name, uint16_t ppp_max_payload)
{
	...
	for(int i=0; i<MAX_CONF_INTERFACES; i++) {
		if(conf_interface_config[i].ifname == NULL)
			break;
		if(!strcmp(conf_interface_config[i].ifname, serv->ifname)) {
	...
	if(interface_conf) {
		if (interface_conf->service_name[0]) {
			int i = 0;
			do {
			    add_tag(pack, sizeof(pack), TAG_SERVICE_NAME, (uint8_t *)interface_conf->service_name[i], strlen(interface_conf->service_name[i]));
			    i++;
...
static void pppoe_send_PADS(struct pppoe_conn_t *conn)
{
	...
	conn->ppp.pppoe_sid = conn->sid;
	memcpy(conn->ppp.client_mac, conn->addr, ETH_ALEN);

  conn->ppp.ses.ppp_info.disc_sock = conn->ppp.disc_sock;
  conn->ppp.ses.ppp_info.ppp_sock = conn->ppp.ppp_sock;
  conn->ppp.ses.ppp_info.ifindex = conn->ppp.ifindex;
  conn->ppp.ses.ppp_info.pppoe_sid = conn->ppp.pppoe_sid;
  memcpy(conn->ppp.ses.ppp_info.hwaddr, conn->ppp.hwaddr, ETH_ALEN);
  memcpy(conn->ppp.ses.ppp_info.addr, conn->ppp.addr, ETH_ALEN); // ppp client mac
  conn->ppp.ses.ppp_info.vid = conn->ppp.vid;
...
static void pppoe_recv_PADI(struct pppoe_serv_t *serv, uint8_t *pack, int size)
{
	...
	struct pppoe_interface_config_t *interface_conf = NULL;
    struct interface_pado_delay_s *pado_data = NULL;
    int iface_pado_delay = 0;

	__sync_add_and_fetch(&stat_PADI_recv, 1);

    hashtable_rc_t hashrt = obj_hashtable_ts_get(ptr_conf_interface_pado_delay, serv->ifname, strlen(serv->ifname), (void * *)&pado_data);
    if(hashrt == HASH_TABLE_OK) {
        iface_pado_delay = pado_data->pado_delay;
        obj_hashtable_ts_nodes_unlock(ptr_conf_interface_pado_delay, serv->ifname, strlen(serv->ifname));
    }

    if(iface_pado_delay == -1)
        return;
  ...
  for(int i=0; i<MAX_CONF_INTERFACES; i++) {
	...
	for (n = 0; n < len; n += sizeof(*tag) + ntohs(tag->tag_len)) {
		...
		switch (ntohs(tag->tag_type)) {
			...
			case TAG_SERVICE_NAME:
			{
				...
				if(interface_conf) {
					service_match = interface_conf->service_name[0] == NULL;
					if (tag->tag_len == 0 && interface_conf->accept_blank_service) {
						service_match = 1;
						break;
					} else if (interface_conf->service_name[0]) {
						int svc_index = 0;
						do {
						    if (ntohs(tag->tag_len) == strlen(interface_conf->service_name[svc_index]) &&
							memcmp(tag->tag_data, interface_conf->service_name[svc_index], ntohs(tag->tag_len)) == 0) {
							    service_match = 1;
							    break;
						    }
						    svc_index++;
						} while(interface_conf->service_name[svc_index]);
					} else
						service_name_tag = tag;
				} 
				else {
	...
	if (interface_conf) {
		if (!service_match && !interface_conf->accept_any_service) {
			if (conf_verbose)
				log_info2("pppoe(1): discarding PADI packet (Service-Name mismatch) inbound-if: %s, MAC: %02x:%02x:%02x:%02x:%02x:%02x",
				    serv->ifname, ethhdr->h_source[0], ethhdr->h_source[1], ethhdr->h_source[2], ethhdr->h_source[3], ethhdr->h_source[4], ethhdr->h_source[5]);
				log_info2(", Service-Name: ");
				if(service_name_tag)
					print_tag_string(service_name_tag);
				log_info2("\n");
			return;
		}
	}
		...
		if (!service_match && !conf_accept_any_service) {
			if (conf_verbose)
				log_info2("pppoe(2): discarding PADI packet (Service-Name mismatch) inbound-if: %s, MAC: %02x:%02x:%02x:%02x:%02x:%02x",
				    serv->ifname, ethhdr->h_source[0], ethhdr->h_source[1], ethhdr->h_source[2], ethhdr->h_source[3], ethhdr->h_source[4], ethhdr->h_source[5]);
				log_info2(", Service-Name: ");
				if(service_name_tag)
					print_tag_string(service_name_tag);
    ...
    if(iface_pado_delay == 0 && pado_delay) {
        iface_pado_delay = pado_delay;
    }
	...
	if (iface_pado_delay) {
		list_for_each_entry(pado, &serv->pado_list, entry) {
			if (memcmp(pado->addr, ethhdr->h_source, ETH_ALEN))
				continue;
			if (conf_verbose)
				log_warn("pppoe: discarding PADI packet (already queued) inbound-if: %s, MAC: %02x:%02x:%02x:%02x:%02x:%02x",
				    serv->ifname, ethhdr->h_source[0], ethhdr->h_source[1], ethhdr->h_source[2], ethhdr->h_source[3], ethhdr->h_source[4], ethhdr->h_source[5]);
				log_info2(", Service-Name: ");
				if(service_name_tag)
					print_tag_string(service_name_tag);
		...
		pado->timer.expire_tv.tv_sec = iface_pado_delay / 1000;
		pado->timer.expire_tv.tv_usec = (iface_pado_delay % 1000) * 1000;
...
static void pppoe_recv_PADR(struct pppoe_serv_t *serv, uint8_t *pack, int size)
{
  ...
  for(int i=0; i<MAX_CONF_INTERFACES; i++) {
	...
	if (interface_conf) {
		if (!service_match && !interface_conf->accept_any_service) {
			if (conf_verbose)
				log_warn("pppoe(1): Service-Name mismatch\n");
		...
		if (!service_match && !conf_accept_any_service) {
			if (conf_verbose)
				log_warn("pppoe(2): Service-Name mismatch\n");
			pppoe_send_err(serv, ethhdr->h_source, host_uniq_tag, relay_sid_tag, CODE_PADS, TAG_SERVICE_NAME_ERROR);
			return;
		}
	}
  ...
  uint8_t *keyCookie = (uint8_t *)ac_cookie_tag->tag_data;
  hashtable_rc_t hash_rc0 = obj_hashtable_ts_get(&serv->cookie_2_conn, keyCookie, COOKIE_LENGTH - 4, (void * *)&conn);
  if (hash_rc0 == HASH_TABLE_OK) {
    if (conn && !conn->ppp.ses.username) {
        __sync_add_and_fetch(&stat_PADR_dup_recv, 1);
        pppoe_send_PADS(conn);
    }
    obj_hashtable_ts_nodes_unlock(&serv->cookie_2_conn, keyCookie, COOKIE_LENGTH - 4);
  }
  ...
  if (conf_max_starting && ap_session_stat.incoming >= conf_max_starting) {
    return;
  }
...
static void pppoe_recv_PADT(struct pppoe_serv_t *serv, uint8_t *pack)
{
  ...
  key64 = (key64 & ~0xffff) | (uint64_t) (sid);
  hashtable_rc_t hash_rc0 = hashtable64_ts_get(&serv->mac_sessid_2_conn, key64, (void * *)&conn);
  if (hash_rc0 == HASH_TABLE_OK) {
    triton_context_call(&conn->ctx, (void (*)(void *))disconnect, conn);
    hashtable64_ts_nodes_unlock(&serv->mac_sessid_2_conn, key64);
...
static void pppoe_recv_PPP(struct pppoe_serv_t *serv, uint8_t *pack, int size)
{
	struct ethhdr *ethhdr = (struct ethhdr *)pack;
	struct pppoe_hdr *hdr = (struct pppoe_hdr *)(pack + ETH_HLEN);
	struct pppoe_conn_t *conn;
  ...
  if (hash_rc0 == HASH_TABLE_OK) {
    int i;
    int proto = ntohs(*(uint16_t*)(pack + sizeof(*hdr) + ETH_HLEN));
    for(i = 0; i < 100; i++) {
      if(!conn->ppp.is_processing)
        break;
      if(i % 100 == 99)
        log_debug("is_processing %s proto:0x%x (%d)\n", conn->ppp.ses.sessionid, proto, i);
      usleep(10);
    }

    //pthread_mutex_lock(&conn->ppp.lock);
    conn->ppp.is_processing = 1;
    //pthread_mutex_unlock(&conn->ppp.lock);
    
    memcpy(conn->ppp.pack, pack, size);

      triton_context_call(&conn->ctx, (triton_event_func)ppp_packet_read, &conn->ppp);

    //triton_context_wakeup(&conn->ctx);
		//ppp_packet_read(&conn->ppp);
...
static int parse_server(const char *opt, int *padi_limit, struct ap_net **net, const char *ifname)
{
	char *ptr, *endptr;
	char name[64];

  struct pppoe_interface_config_t *interface_conf;

  if(strstr(opt, "service-name=") || strstr(opt, "accept-any-service=") || strstr(opt, "accept-blank-service=")) {
    int i;
    for(i=0; i<MAX_CONF_INTERFACES; i++) {
      if(conf_interface_config[i].ifname == NULL) {
        conf_interface_config[i].ifname = _strdup(ifname);
        interface_conf = &conf_interface_config[i];
        break;
      }
    }
	...
	while (*opt == ',') {
		...
		if (!strncmp(opt, "padi-limit=", sizeof("padi-limit=") - 1)) {
			...
			*net = ap_net_find(name);
			if (!*net)
				goto out_err;
		} else if(!strncmp(opt, "service-name=", sizeof("service-name=") - 1)) {
			char service_name[512];
			ptr++;
			for (endptr = ptr + 1; *endptr && strncmp(endptr, ",accept-any-service", sizeof(",accept-any-service") -  1)
					&& strncmp(endptr, ",accept-blank-service", sizeof(",accept-blank-service") -  1); endptr++);
			if (endptr - ptr >= sizeof(service_name))
				goto out_err;
			memcpy(service_name, ptr, endptr - ptr);
			service_name[endptr - ptr] = 0;
			...
			opt = opt + sizeof("service-name=") - 1;
			char *conf_service_name_string = _strdup(service_name);
			...
			opt = endptr;
			printf(" %s,service-name=%s\n", interface_conf->ifname, service_name);
		} else if (!strncmp(opt, "accept-any-service=", sizeof("accept-any-service=") - 1)) {
			interface_conf->accept_any_service = strtol(ptr + 1, &endptr, 10);
			if (*endptr != 0 && *endptr != ',')
				goto out_err;
			opt = endptr;
			printf(" %s,accept-any-service=%d\n", interface_conf->ifname, interface_conf->accept_any_service);
		} else if (!strncmp(opt, "accept-blank-service=", sizeof("accept-blank-service=") - 1)) {
			interface_conf->accept_blank_service = strtol(ptr + 1, &endptr, 10);
			if (*endptr != 0 && *endptr != ',')
				goto out_err;
			opt = endptr;
...
static void __pppoe_server_start(const char *ifname, const char *opt, void *cli, int parent_ifindex, int vid, int vlan_mon)
{
	...
	if (parse_server(opt, &padi_limit, &net, ifname)) {
		if (cli)
  ...
  obj_hashtable_ts_init(&serv->cookie_2_conn, 1000, NULL, NULL, NULL, "cookie_2_conn");
  hashtable64_ts_init(&serv->mac_sessid_2_conn, 1000, NULL, NULL, "mac_sessid_2_conn");
  ...
  serv->ppp_sock = pppoe_ppp_ses_start(serv);
  if (serv->ppp_sock < 0) {
    log_error("pppoe: %s: failed to create session socket\n", ifname);
    triton_context_unregister(&serv->ctx);
    goto out_err;
  }
...
void _server_stop(struct pppoe_serv_t *serv)
{
  ...
  while ((num_elements < serv->cookie_2_conn.num_elements) && (i < serv->cookie_2_conn.size)) {
    pthread_mutex_lock(&serv->cookie_2_conn.lock_nodes[i]);
    if (serv->cookie_2_conn.nodes[i] != NULL) {
      node = serv->cookie_2_conn.nodes[i];

      while (node) {
        conn = (struct pppoe_conn_t *)node->data;
        triton_context_call(&conn->ctx, (triton_event_func)_conn_stop, conn);
        node = node->next;
        num_elements++;
      }
    }
    pthread_mutex_unlock(&serv->cookie_2_conn.lock_nodes[i]);
    i++;
  }
...
void pppoe_vlan_mon_notify(int ifindex, int vid, int vlan_ifindex)
{
	...
	if (vlan_ifindex) {
		...
		log_info2("pppoe(1): create vlan %s parent %s\n", ifname, ifr.ifr_name);
		...
		log_info2("pppoe(2): create vlan %s parent %s\n", ifname, ifr.ifr_name);
...
static void load_pado_delay_iface()
{
    struct interface_pado_delay_s *node_data = NULL;
    if(ptr_conf_interface_pado_delay) {
        size_t                                 n;
        obj_hash_node_t                     *node;
        struct dpado_range_t *r;

        for (n = 0; n < ptr_conf_interface_pado_delay->num_elements; ++n) {
            pthread_mutex_lock (&ptr_conf_interface_pado_delay->lock_nodes[n]);
            node = ptr_conf_interface_pado_delay->nodes[n];

            while (node) {
              node = node->next;
              node_data = (struct interface_pado_delay_s *)node->data;
              while (!list_empty(&node_data->dpado_range_list)) {
                r = list_entry(node_data->dpado_range_list.next, typeof(*r), entry);
                list_del(&r->entry);
                _free(r);
              }
            }
            pthread_mutex_unlock (&ptr_conf_interface_pado_delay->lock_nodes[n]);
        }

        obj_hashtable_ts_destroy(ptr_conf_interface_pado_delay);
    }
    else
        ptr_conf_interface_pado_delay = &conf_interface_pado_delay;
    
    obj_hashtable_ts_init(ptr_conf_interface_pado_delay, 1000, NULL, pado_iface_cmpfunc_obj, NULL, "conf_interface_pado_delay");

    struct conf_sect_t *s = conf_get_section("pppoe");
    struct conf_option_t *opt;

    list_for_each_entry(opt, &s->items, entry) {
        if (!strcmp(opt->name, "pado-delay-iface")) {
            if (opt->val)
                dpado_parse_iface(opt->val);
        }
    }
}
...
static void load_config(void)
{
	...
	if (opt) {
		...
		conf_ac_name = _strdup("micro-bng");
...
#define PADI_ARRAY_COUNT 5
static void *padi_per_sec_thread(void *a)
{

  unsigned long padi_array[PADI_ARRAY_COUNT];
  int i = 0;
  unsigned long sum;

  unsigned long prev_PADI_recv = 0;

  memset(padi_array, 0, sizeof(padi_array));

  while(1) {

    padi_array[i++] = stat_PADI_recv - prev_PADI_recv;
    prev_PADI_recv = stat_PADI_recv;

    sum = 0;
    for(int x=0; x<PADI_ARRAY_COUNT; x++) {
      sum += padi_array[x];
    }
    
    PADI_per_Second = sum / PADI_ARRAY_COUNT;
    
    if(i >= PADI_ARRAY_COUNT) i = 0;
    sleep(1);
  }
  return NULL;
}
...
static void pppoe_init(void)
{
  ...
  pthread_t padi_thread;
	...
	memset(conf_interface_config, 0, sizeof(conf_interface_config));

	load_config();
	load_interfaces();

    pthread_create(&padi_thread, NULL, padi_per_sec_thread, NULL);
