...
static void release_ipv6_pppoe_policer_vpp(struct ppp_ipv6cp_t *ipv6cp)
{

  struct ap_session *sess = &(ipv6cp->ppp->ses);
  ...
  // remove dp routes
  if(sess->ipv6_dp) {
    log_ppp_debug(" [del ipv6_dp] IPV6CP PPPOE SID : %04x\n", ipv6cp->ppp->pppoe_sid);
    list_for_each_entry(p, &sess->ipv6_dp->prefix_list, entry) {
      vpp_api_ipv6_route_add_del(p->addr.s6_addr, p->prefix_len, NULL, pppoe_sw_if_index, 0, 0);

      log_ppp_debug(" [del ipv6_dp] DEL PPPOE SESSION : %04x\n", ipv6cp->ppp->pppoe_sid);
      vpp_api_ipv6_pppoe_add_del_session(p->addr.s6_addr, p->prefix_len, ipv6cp->ppp->client_mac, ipv6cp->ppp->pppoe_sid, "", ~0, "", 0, 0);
  ...
  /*log_ppp_debug(" [del ipv6] DEL PPPOE SESSION : %04x\n", ipv6cp->ppp->pppoe_sid);
  vpp_api_ipv6_pppoe_add_del_session(NULL, 128, ipv6cp->ppp->client_mac, ipv6cp->ppp->pppoe_sid, "", ~0, "", 0, 0);*/
...
void ipv6cp_layer_free(struct ppp_layer_data_t *ld)
{
  ...
  if(ipv6cp->started)
    release_ipv6_pppoe_policer_vpp(ipv6cp);
...
static void setup_ipv6_pppoe_policer_vpp(struct ppp_ipv6cp_t *ipv6cp)
{
  ...
  if(sess->vpp_pppoe_sw_if_index != ~0) {
    log_ppp_debug("setup_ipv6_pppoe_policer_vpp: ipv6 route add\n");
    if (sess->ipv6 && !list_empty(&sess->ipv6->addr_list)) {
      ...
      if (a->prefix_len == 0) {
        ...
        vpp_api_ipv6_route_add_del(addr.s6_addr, 128, NULL, sess->vpp_pppoe_sw_if_index, 0, 1);
	...
	log_ppp_debug(" IPV6 PPPOE SID : %04x\n", ipv6cp->ppp->pppoe_sid);
	log_ppp_debug(" max_up: %d, max_down: %d, min_up: %d, min_down: %d\n", sess->microbng_bandwidth_max_up,
		sess->microbng_bandwidth_max_down, sess->microbng_bandwidth_min_up,
		sess->microbng_bandwidth_min_down);

    //pppoe_sw_if_index = vpp_api_ipv6_pppoe_add_del_session(addr.s6_addr, ipv6cp->ppp->client_mac, ipv6cp->ppp->pppoe_sid, 1);
  vpp_api_ipv6_pppoe_add_del_session(addr.s6_addr, 128, ipv6cp->ppp->client_mac, ipv6cp->ppp->pppoe_sid, sess->sessionid, ~0, sess->username?: "", ipv6cp->ppp->vid, 1);

	//printf("pppoe sw_if_name: %u\n", pppoe_sw_if_index);

	/*len = u_parse_ip6cidr("::1/128", &base_ip6addr, &suffix);
	base_ip6addr.s6_addr[7] += pppoe_sw_if_index;
...
static void ipv6cp_layer_up(struct ppp_fsm_t *fsm)
{
    ...
    sess->ipv6_started = 1;
    if(sess->ipv6) {
    	setup_ipv6_pppoe_policer_vpp(ipv6cp);
    }
...
static void ipv6cp_layer_finished(struct ppp_fsm_t *fsm)
{
	...
	log_ppp_debug("ipv6cp_layer_finished(2)\n");
...
struct ipv6cp_option_t *ipv6cp_find_option(struct ppp_t *ppp, struct ipv6cp_option_handler_t *h)
{
	...
	if (!ld) {
		log_ppp_error("ipv6cp: layer data not found\n");
		return NULL;
	}
	...
	log_ppp_error("ipv6cp: option not found, ignoring recoverable option event\n");
	return NULL;
}
