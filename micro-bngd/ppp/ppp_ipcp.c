...
extern u32 sw_if_index_loop0;
extern u32 download_classify_table_index;
extern u32 ipv6_classify_table_index;
extern u32 upload_classify_table_index;

extern int dflt_down_speed;
extern int dflt_up_speed;
extern double down_burst_factor;
extern double up_burst_factor;
...
static void release_pppoe_policer_vpp(struct ppp_ipcp_t *ipcp)
{
	char pppoe_session_name[64], policy_name[64];
	struct in_addr base_addr;
	uint8_t suffix;
	
	struct ap_session *sess = &(ipcp->ppp->ses);
	u32 pppoe_sw_if_index = sess->vpp_pppoe_sw_if_index;
	u32 policer_index;
  ...
  struct ipv6db_addr_t *p;
	...
	if(pppoe_sw_if_index == ~0)
		return;
	
	uint32_t ip_address = sess->clientip;
	log_ppp_debug(" [del] PPPOE SID : %04x\n", ipcp->ppp->pppoe_sid);
	log_ppp_debug(" [del] %d, %d, %d, %d\n", sess->microbng_bandwidth_max_up,
		sess->microbng_bandwidth_max_down, sess->microbng_bandwidth_min_up,
		sess->microbng_bandwidth_min_down);

	//printf("pppoe sw_if_name: %u\n", pppoe_sw_if_index);
  if(sess->ipv6_started) {
    if (sess->ipv6 && !list_empty(&sess->ipv6->addr_list)) {
      log_ppp_debug(" [del] IPV6 PPPOE SID : %04x\n", ipcp->ppp->pppoe_sid);
        a = list_entry(sess->ipv6->addr_list.next, typeof(*a), entry);
      if (a->prefix_len == 0) {
        //return;
      } else {
      
        build_ip6_addr(a, sess->ipv6->peer_intf_id, &addr);
        /*for(int i=0; i<8; i++) {
          printf("%x%02x:", addr.s6_addr[i*2], addr.s6_addr[i*2+1]);
        }
        printf("\n");*/

        vpp_api_ipv6_route_add_del(addr.s6_addr, 128, NULL, pppoe_sw_if_index, 0, 0);

        /* download ipv6 policer */
        u8 match_down[64];
        memset(match_down, 0, sizeof(match_down));
        memcpy(&match_down[38], addr.s6_addr, 16);
        vpp_api_classify_add_del_session(ipv6_classify_table_index, sess->vpp_policer_id_down, match_down, sizeof(match_down), 0);
      }

      log_ppp_debug(" [del] DEL PPPOE SESSION : %04x\n", ipcp->ppp->pppoe_sid);
      vpp_api_ipv6_pppoe_add_del_session(addr.s6_addr, 128, ipcp->ppp->client_mac, ipcp->ppp->pppoe_sid, "", ~0, "", 0, 0);
    }
  }
  
	vpp_api_ip_route_add_del(ip_address, 32, 0, pppoe_sw_if_index, 0, 0);

  if(sess->is_radius_ip) {
    struct in_addr ipv4addr;
    char ipv4str[INET_ADDRSTRLEN];

    ipv4addr.s_addr = ip_address;
    ...
    if (inet_ntop(AF_INET, &ipv4addr, ipv4str, sizeof(ipv4str)) == NULL) {
      ...
      char cmd[80];
      snprintf(cmd, sizeof(cmd), "ip address del %s/32 dev frr0", ipv4str);
      system(cmd);
    }
  }
  
	/*size_t len = u_parse_ip4cidr("127.0.0.1/32", &base_addr, &suffix);
	u32 pppoe_local_ip = ntohl( (htonl(base_addr.s_addr) + pppoe_sw_if_index) % 0x7FFFFFFF );
	vpp_api_sw_interface_add_del_address(pppoe_sw_if_index, pppoe_local_ip , suffix, 0);*/

	/* download policer */
	policer_index = sess->vpp_policer_id_down;
	snprintf(policy_name, sizeof(policy_name), "down%s", sess->sessionid);
	vpp_api_policer_add_del(policy_name, sess->microbng_bandwidth_max_down, sess->microbng_bandwidth_max_down * down_burst_factor * 100, 0, 0, 0);
	u8 match_down[48];
	memset(match_down, 0, sizeof(match_down));
	...
	/* upload policer */
	policer_index = sess->vpp_policer_id_up;
	snprintf(policy_name, sizeof(policy_name), "up%s", sess->sessionid);
	vpp_api_policer_add_del(policy_name, sess->microbng_bandwidth_max_up, sess->microbng_bandwidth_max_up * up_burst_factor * 100, 0, 0, 0);
	u8 match_up[16];
	memset(match_up, 0, sizeof(match_up));
	memcpy(&match_up[6], ipcp->ppp->client_mac, sizeof(ipcp->ppp->client_mac));
	vpp_api_classify_add_del_session(upload_classify_table_index, policer_index, match_up, sizeof(match_up), 0);
	//vpp_api_policer_classify_set_interface(pppoe_sw_if_index, 2 /*upload_classify_table_index*/, ~0, ~0, 0);

	vpp_api_pppoe_add_del_session(ip_address, ipcp->ppp->client_mac, ipcp->ppp->pppoe_sid, "", ~0, "", 0, 0);
...
void ipcp_layer_free(struct ppp_layer_data_t *ld)
{
	...
	release_pppoe_policer_vpp(ipcp);
...
static void setup_pppoe_policer_vpp(struct ppp_ipcp_t *ipcp)
{
	char pppoe_session_name[64], policy_name[64];
	struct in_addr base_addr;
	uint8_t suffix;
  ...
  if(sess->microbng_bandwidth_max_down == 0)
      sess->microbng_bandwidth_max_down = dflt_down_speed;
  if(sess->microbng_bandwidth_max_up == 0)
    sess->microbng_bandwidth_max_up = dflt_up_speed;

	uint32_t ip_address = sess->ipv4->peer_addr;
	log_ppp_debug(" PPPOE SID : 0x%04x, vid : %d, Acct-Session_id : %s, username : %s\n", ipcp->ppp->pppoe_sid, ipcp->ppp->vid, sess->sessionid, sess->username?: "");
	log_ppp_debug(" max_up: %d, max_down: %d, min_up: %d, min_down: %d, down_burst_factor: %f, up_burst_factor: %f\n", sess->microbng_bandwidth_max_up,
		sess->microbng_bandwidth_max_down, sess->microbng_bandwidth_min_up, sess->microbng_bandwidth_min_down,
		down_burst_factor, up_burst_factor);

	pppoe_sw_if_index = vpp_api_pppoe_add_del_session(ip_address, ipcp->ppp->client_mac, ipcp->ppp->pppoe_sid, sess->sessionid, ~0, sess->username?: "", ipcp->ppp->vid, 1);
	//printf("pppoe sw_if_name: %u\n", pppoe_sw_if_index);
  if(pppoe_sw_if_index == ~0) {
    //I try to remove and create
    pppoe_sw_if_index = vpp_api_pppoe_add_del_session(ip_address, ipcp->ppp->client_mac, ipcp->ppp->pppoe_sid, sess->sessionid, ~0, sess->username?: "", ipcp->ppp->vid, 0);
    pppoe_sw_if_index = vpp_api_pppoe_add_del_session(ip_address, ipcp->ppp->client_mac, ipcp->ppp->pppoe_sid, sess->sessionid, ~0, sess->username?: "", ipcp->ppp->vid, 1);
    //printf("recreted pppoe sw_if_name: %u\n", pppoe_sw_if_index);
    if(pppoe_sw_if_index == ~0)
      return;
  }

  vpp_api_ip_route_add_del(ip_address, 32, 0, pppoe_sw_if_index, 0, 1);

  if(sess->ipv4->is_radius == 1) {
    struct in_addr ipv4addr;
    char ipv4str[INET_ADDRSTRLEN];
    ...
    if (inet_ntop(AF_INET, &ipv4addr, ipv4str, sizeof(ipv4str)) == NULL) {
      ...
      char cmd[80];
      snprintf(cmd, sizeof(cmd), "ip address add %s/32 dev frr0", ipv4str);
      system(cmd);
    }
	...
	/*len = u_parse_ip4cidr("127.0.0.2/32", &base_addr, &suffix);
	u32 pppoe_local_ip = ntohl( (htonl(base_addr.s_addr) + pppoe_sw_if_index) % 0x7FFFFFFF );
	vpp_api_sw_interface_add_del_address(pppoe_sw_if_index, pppoe_local_ip , suffix, 1);*/

  vpp_api_sw_interface_set_unnumbered(pppoe_sw_if_index, sw_if_index_loop0, 1);

  if(sess->ipv6_started && sess->download_ipv6_classify_table_index == ~0) {
    if (sess->ipv6 && !list_empty(&sess->ipv6->addr_list)) {
        a = list_entry(sess->ipv6->addr_list.next, typeof(*a), entry);
      if (a->prefix_len == 0) {
        //return;
      } else {
        log_ppp_debug("ipv6 prefix_len %d\n", a->prefix_len);
        build_ip6_addr(a, sess->ipv6->peer_intf_id, &addr);
        /*for(int i=0; i<8; i++) {
      		printf("%x%02x:", addr.s6_addr[i*2], addr.s6_addr[i*2+1]);
      	}
        ...
        vpp_api_ipv6_route_add_del(addr.s6_addr, 128, NULL, pppoe_sw_if_index, 0, 1);

        /*struct in6_addr base_ip6addr;
        len = u_parse_ip6cidr("::1/128", &base_ip6addr, &suffix);
        *(uint64_t *)(base_ip6addr.s6_addr) = htobe64(be64toh(*(uint64_t *)(base_ip6addr.s6_addr)) + pppoe_sw_if_index);
        vpp_api_sw_interface_add_del_ipv6_address(pppoe_sw_if_index, base_ip6addr.s6_addr, 64, 1);*/
      }
    }

    sess->download_ipv6_classify_table_index = ipv6_classify_table_index;
  }

	/* download policer */
	snprintf(policy_name, sizeof(policy_name), "down%s", sess->sessionid);
	u32 policer_index = vpp_api_policer_add_del(policy_name, sess->microbng_bandwidth_max_down, sess->microbng_bandwidth_max_down * down_burst_factor * 100, 0, 0, 1);
	u8 match_down[48];
	memset(match_down, 0, sizeof(match_down));
	match_down[30] = ( ip_address ) & 0xFF;
	match_down[31] = ( ip_address >> 8 ) & 0xFF;
  ...
  log_ppp_debug("match ip_address %d.%d.%d.%d\n", match_down[30], match_down[31], match_down[32], match_down[33]);
	...
	sess->vpp_policer_id_down = policer_index;

	/* upload policer */
	snprintf(policy_name, sizeof(policy_name), "up%s", sess->sessionid);
	policer_index = vpp_api_policer_add_del(policy_name, sess->microbng_bandwidth_max_up, sess->microbng_bandwidth_max_up * up_burst_factor * 100, 0, 0, 1);
	u8 match_up[16];
	memset(match_up, 0, sizeof(match_up));
	memcpy(&match_up[6], ipcp->ppp->client_mac, sizeof(ipcp->ppp->client_mac));
	vpp_api_classify_add_del_session(upload_classify_table_index, policer_index, match_up, sizeof(match_up), 1);
	vpp_api_policer_classify_set_interface(pppoe_sw_if_index, upload_classify_table_index, upload_classify_table_index, ~0, 1);
	sess->vpp_policer_id_up = policer_index;

	sess->vpp_pppoe_sw_if_index = pppoe_sw_if_index;
	sess->clientip = ip_address;

    if(sess->download_ipv6_classify_table_index != ~0) {
        /* download ipv6 policer */
        u8 match_down[64];
        memset(match_down, 0, sizeof(match_down));
        memcpy(&match_down[38], addr.s6_addr, 16);
        vpp_api_classify_add_del_session(ipv6_classify_table_index, sess->vpp_policer_id_down, match_down, sizeof(match_down), 1);
    }
