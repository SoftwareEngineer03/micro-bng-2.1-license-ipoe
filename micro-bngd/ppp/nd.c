...
static uint16_t
icmpv6_ip6_checksum (struct in6_addr *src_addr, struct in6_addr *dest_addr, uint8_t* up, uint32_t len)
{
  size_t i;
  uint16_t *sp;
  uint32_t sum = 0;
...
static void ipv6_nd_send_ra(struct ipv6_nd_handler_t *h, struct sockaddr_in6 *dst_addr)
{
  ...
  *proto = htons(PPP_IPV6);
  //log_ppp_debug("+ipv6_nd_send_ra proto: 0x%x\n", ntohs(*proto));

  int ip6_offset = sizeof(uint16_t);
  struct ip6_hdr *ipv6_h = (struct ip6_hdr*)(data + ip6_offset);
  int ipv6_header_length = sizeof(struct ip6_hdr);

  ipv6_h->ip6_flow = htonl((6 << 28) | (0 << 20) | 0);
  //log_ppp_debug("+ipv6_nd_send_ra ip6_flow: 0x%08x, 0x%08x, length: %ld\n", (6 << 28) | (0 << 20) | 0, ipv6_h->ip6_flow, endptr - buf);
  ipv6_h->ip6_plen = htons(endptr - buf);
  ipv6_h->ip6_nxt = IPPROTO_ICMPV6;
  ipv6_h->ip6_hops = 255;
  ipv6_h->ip6_src.s6_addr32[0] = htonl(0xfe800000);
  memcpy(ipv6_h->ip6_src.s6_addr + 8, &ses->ipv6->intf_id, 8);
  memcpy(&ipv6_h->ip6_dst, &dst_addr->sin6_addr, sizeof(dst_addr->sin6_addr));



  adv->nd_ra_cksum = icmpv6_ip6_checksum(&ipv6_h->ip6_src, &ipv6_h->ip6_dst, buf, endptr - buf); // Calculate checksum
  //printf("ICMPv6 Checksum: %04X, Length: %ld\n", ntohs(adv->nd_ra_cksum), endptr - buf);  // Print checksum



  memcpy(data + ip6_offset + ipv6_header_length, buf, endptr - buf);

  data_size = ip6_offset + ipv6_header_length + (endptr - buf);

  ppp_data_send(&ses->ppp_info, data, data_size);
...
int __export ppp_ipv6_nd_read(struct ppp_t *ppp)
{
...
//	socklen_t addr_len = sizeof(addr);
  ...
  //log_ppp_debug("ppp_ipv6_nd_read.........\n");
  ...
  //log_ppp_debug("ppp_ipv6_nd_read proto: 0x%x\n", proto);

  int ip6_offset = sizeof(*hdr) + ETH_HLEN + sizeof(proto);
  const struct ip6_hdr *ipv6_h = (struct ip6_hdr*)(pack + ip6_offset);
  int ipv6_header_length = sizeof(struct ip6_hdr);

  //log_ppp_debug("ppp_ipv6_nd_read ip6_nxt: %d, %d\n", ipv6_h->ip6_nxt, ipv6_header_length);

  icmph = (struct icmp6_hdr *)(pack + ip6_offset + ipv6_header_length);
  n = ntohs(ipv6_h->ip6_plen); // Payload length

  if (n < sizeof(*icmph)) {
    log_ppp_warn("ppp_ipv6_nd_read: received short icmp packet (%i)\n", n);
    return 0;
  }

  if (icmph->icmp6_type != ND_ROUTER_SOLICIT) {
    log_ppp_warn("ppp_ipv6_nd_read: received unexcpected icmp packet (%i)\n", icmph->icmp6_type);
    return 0;
  }

  if (!IN6_IS_ADDR_LINKLOCAL(&ipv6_h->ip6_src)) {
    log_ppp_warn("ppp_ipv6_nd_read: received icmp packet from non link-local address\n");
    return 0;
  }
  ...
  memcpy(&addr.sin6_addr, &ipv6_h->ip6_src, sizeof(addr.sin6_addr));
...
static int ipv6_nd_start(struct ap_session *ses)
{
  ...
  log_ppp_debug("+ipv6_nd: ipv6_nd_start.........\n");
