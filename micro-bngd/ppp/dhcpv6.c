...
static void insert_dp_routes(struct ap_session *ses, struct dhcpv6_pd *pd, struct in6_addr *addr)
{
    ...
    vpp_api_ipv6_route_add_del(p->addr.s6_addr, p->prefix_len, NULL, ses->vpp_pppoe_sw_if_index, 0, 1);

    vpp_api_ipv6_pppoe_add_del_session(p->addr.s6_addr, p->prefix_len, ses->ppp_info.addr, ses->ppp_info.pppoe_sid, ses->sessionid, ~0, ses->username?: "", ses->ppp_info.vid, 1);

    /* download ipv6 policer */
    u8 match_down[64];
    memset(match_down, 0, sizeof(match_down));
    memcpy(&match_down[38], p->addr.s6_addr, 16);
    vpp_api_classify_add_del_session(ses->download_ipv6_classify_table_index, ses->vpp_policer_id_down, match_down, sizeof(match_down), 1);
...
static uint16_t
udp_ip6_checksum (struct in6_addr *src_addr, struct in6_addr *dest_addr, uint8_t* up, uint32_t len)
{
  size_t i;
  uint16_t *sp;
  uint32_t sum;
  ...
  memcpy(phu.ph.ph_src, src_addr->s6_addr, 16);
  memcpy(phu.ph.ph_dst, dest_addr->s6_addr, 16);
...
static void get_ipv6_address(const char *interface_name, struct ip6_hdr *ip6hdr) {
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        // Check if the interface name matches and it is an IPv6 address
        if (ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_INET6 && 
            ...
            if (inet_ntop(AF_INET6, &((struct sockaddr_in6 *)ifa->ifa_addr)->sin6_addr, addr, sizeof(addr)) != NULL) {
                ...
                // Set the source address in the ip6_hdr structure
                inet_pton(AF_INET6, addr, &ip6hdr->ip6_src);
                return; // Exit after finding and setting the address
            } else {
                perror("inet_ntop");
            }
        }
...
static void dhcpv6_send_reply(struct dhcpv6_packet *req, struct dhcpv6_pd *pd, int code)
{
  ...
  //log_ppp_debug("+dhcpv6_send_reply proto: 0x%x\n", proto);

  int ip6_offset = sizeof(*hdr) + ETH_HLEN + sizeof(proto);
  struct ip6_hdr *ipv6_h = (struct ip6_hdr *)(pack + ip6_offset);
  int ipv6_header_length = sizeof(struct ip6_hdr);

  ipv6_h->ip6_dst = ipv6_h->ip6_src;
  //get_ipv6_address(req->ppp->ses.ifname, ipv6_h);
  ipv6_h->ip6_src.s6_addr32[0] = htonl(0xfe800000);
  memcpy(ipv6_h->ip6_src.s6_addr + 8, &ses->ipv6->intf_id, 8);

  ipv6_h->ip6_hops = 255;

  //log_ppp_debug("+dhcpv6_send_reply ip6_nxt: %d, %d, %d\n", ipv6_h->ip6_nxt, IPPROTO_UDP, ipv6_header_length);

  struct udphdr *udp_header = (struct udphdr *)(pack + ip6_offset + ipv6_header_length);
  uint16_t src_port = udp_header->uh_sport;
  udp_header->uh_sport = udp_header->uh_dport;
  udp_header->uh_dport = src_port;
  udp_header->uh_sum = 0;

  payload_length = sizeof(struct udphdr) + reply->endptr - (void *)reply->hdr;
  udp_header->uh_ulen = ntohs(payload_length);
  ipv6_h->ip6_plen = udp_header->uh_ulen;

  //log_ppp_debug("+dhcpv6_send_reply payload length: %d\n", payload_length);

  int ppp_ip6_udp_len = sizeof(proto) + ipv6_header_length + sizeof(struct udphdr);
  memcpy(buf, pack + sizeof(*hdr) + ETH_HLEN, ppp_ip6_udp_len);
  memcpy(buf + ppp_ip6_udp_len, reply->hdr, reply->endptr - (void *)reply->hdr);

  uint16_t checksum = udp_ip6_checksum(&ipv6_h->ip6_src, &ipv6_h->ip6_dst, buf + sizeof(proto) + ipv6_header_length,
    payload_length); // Calculate checksum
  //printf("UDP Checksum: %04X, Length: %d\n", ntohs(checksum), payload_length);  // Print checksum

  udp_header = (struct udphdr *)(buf + sizeof(proto) + ipv6_header_length);
  udp_header->uh_sum = checksum;

  buf_size = ppp_ip6_udp_len + reply->endptr - (void *)reply->hdr;

  //ppp_unit_send(req->ppp, reply->hdr, reply->endptr - (void *)reply->hdr);
  ppp_unit_send(req->ppp, buf, buf_size);
...
static void dhcpv6_send_reply2(struct dhcpv6_packet *req, struct dhcpv6_pd *pd, int code)
{
  ...
  proto = ntohs(*(uint16_t*)(pack + sizeof(*hdr) + ETH_HLEN));
  //log_ppp_debug("+dhcpv6_send_reply2 proto: 0x%x\n", proto);
  ...
  //log_ppp_debug("+dhcpv6_send_reply2 ip6_nxt: %d, %d, %d\n", ipv6_h->ip6_nxt, IPPROTO_UDP, ipv6_header_length);
  ...
  //log_ppp_debug("+dhcpv6_send_reply2 payload length: %d\n", payload_length);
...
int __export ppp_dhcpv6_read(struct ppp_t *ppp)
{
	//struct dhcpv6_pd *pd = container_of(h, typeof(*pd), hnd);
	struct ap_session *ses = &ppp->ses;
	struct sockaddr_in6 addr;
	//socklen_t len = sizeof(addr);
	struct dhcpv6_packet *pkt;
...
//while (1) {
  ...
  proto = ntohs(*(uint16_t*)(pack + sizeof(*hdr) + ETH_HLEN));
  //log_ppp_debug("ppp_dhcpv6_read proto: 0x%x\n", proto);

  int ip6_offset = sizeof(*hdr) + ETH_HLEN + sizeof(proto);
  const struct ip6_hdr *ipv6_h = (struct ip6_hdr*)(pack + ip6_offset);
  int ipv6_header_length = sizeof(struct ip6_hdr);

  if (!IN6_IS_ADDR_LINKLOCAL(&ipv6_h->ip6_src))
    goto cont;
  
  //log_ppp_debug("ppp_dhcpv6_read ip6_nxt: %d, %d, %d\n", ipv6_h->ip6_nxt, IPPROTO_UDP, ipv6_header_length);
  if(ipv6_h->ip6_nxt == IPPROTO_UDP) {
    const struct udphdr *udp_header = (const struct udphdr *)(pack + ip6_offset + ipv6_header_length);
    uint16_t src_port = ntohs(udp_header->uh_sport);

    if (src_port != DHCPV6_CLIENT_PORT)
      goto cont;

    int udp_header_length = sizeof(struct udphdr);
    int payload_length = ntohs(ipv6_h->ip6_plen) - udp_header_length; // Payload length calculation

    memcpy(buf, pack + ip6_offset + ipv6_header_length + udp_header_length, payload_length);

    //log_ppp_debug("ppp_dhcpv6_read payload length: %d\n", payload_length);

		pkt = dhcpv6_packet_parse(buf, payload_length);
		if (!pkt || !pkt->clientid) {
			goto cont;
		}

    memcpy(&addr.sin6_addr, &ipv6_h->ip6_src, sizeof(ipv6_h->ip6_src));
