...
__export u32 sw_if_index_loop0 = ~0;
__export u32 download_classify_table_index = 0;
__export u32 ipv6_classify_table_index = 1;
__export u32 upload_classify_table_index = 2;
...
int main(int _argc, char **_argv)
{
  ...
  u8 mask_up[] = {0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x00};
  u8 delegate6[16];
  int ipv6_dp_prefix_len = get_ipv6_dp_prefix_len();

  memset(mask_ipv6_down, 0, sizeof(mask_ipv6_down));
  memset(delegate6, 0xFF, sizeof(delegate6));
...
/*  vpp_api_classify_add_del_table(1, 2, mask_down, sizeof(mask_down), 0, 0);
  ...
  for(int i=0; i<8; i++) {
    printf("%02x%02x:", delegate6[i*2], delegate6[i*2+1]);
  }
  printf("\n");
