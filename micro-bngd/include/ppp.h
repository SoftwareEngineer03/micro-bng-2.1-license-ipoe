...
#define MAX_VPP_PPPOE_SESSION_IF 128
struct vpp_pppoe_session_if_t
{
	int vid;
...
int ppp_dhcpv6_read(struct ppp_t *ppp);
int ppp_ipv6_nd_read(struct ppp_t *ppp);

int ppp_packet_read(struct ppp_t *ppp);
...
int ppp_data_send(struct ppp_info_s *ppp, void *data, int size);
