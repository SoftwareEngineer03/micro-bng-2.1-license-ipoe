...
EGIT_REPO_URI="git://micro-bng.git.sourceforge.net/gitroot/micro-bng/micro-bng"

DESCRIPTION="PPtP/L2TP/PPPoE Server for Linux"
SRC_URI=""
HOMEPAGE="http://micro-bng.sourceforge.net/"
...
src_prepare() {
	sed -i -e "/mkdir/d" "${S}/micro-bngd/CMakeLists.txt"
	sed -i -e "/echo/d" "${S}/micro-bngd/CMakeLists.txt"
...
src_install() {
	...
	exeinto /etc/init.d
	newexe "${S}/contrib/gentoo/net-dialup/micro-bng/files/micro-bngd-init" micro-bngd

	insinto /etc/conf.d
	newins "${S}/contrib/gentoo/net-dialup/micro-bng/files/micro-bngd-confd" micro-bngd

	dodir /var/log/micro-bng
	dodir /var/run/micro-bng
