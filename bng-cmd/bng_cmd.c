...
static ssize_t bng_sendmsg(int fd, const struct msghdr *mhdr)
{
	ssize_t sndlen;
...
static ssize_t bng_send(int fd, const void *buf, size_t buflen)
{
	ssize_t sndlen;
...
static ssize_t bng_read(int fd, void *buf, size_t buflen)
{
	ssize_t res;
...
static int bng_talk(int cmdfd, int accelfd, const struct msghdr *mhdr,
	...
	for (;;) {
		...
		if (mhdr == NULL && cmdfd < 0 && bytes_wr == bytes_rd
			...
			/* Nothing to read anymore and all data have
			   been sent. Send "exit" so that micro-bng
		...
		if (mhdr || bytes_wr < bytes_rd) {
			/* Data need to be sent to micro-bng */
			FD_SET(accelfd, &wfds);
			fdmax = (accelfd > fdmax) ? accelfd : fdmax;
		}
		if (start_read) {
			/* Wait for data from micro-bng */
			FD_SET(accelfd, &rfds);
			fdmax = (accelfd > fdmax) ? accelfd : fdmax;
		}
		if (cmdfd >= 0 && bytes_rd < sizeof(transbuf)) {
			/* Forward data from input stream to micro-bng */
			FD_SET(cmdfd, &rfds);
		...
		if (FD_ISSET(accelfd, &wfds)) {
			/* Send request to micro-bng */
			res = 0;
			if (mhdr) {
				res = bng_sendmsg(accelfd, mhdr);
				if (res < 0) {
					if (res == -1)
						err = XSTATUS_ARGVLEN;
					else
				...
				res = bng_send(accelfd, transbuf + bytes_wr,
						 bytes_rd - bytes_wr);
				if (res < 0) {
					err = XSTATUS_INTERNAL;
					break;
			...
			if (res > 0)
				/* Start reading micro-bng's response
				   as soon as data have been sent */
				start_read = true;
		}

		if (FD_ISSET(accelfd, &rfds)) {
			/* Read answer from micro-bng */
			res = bng_read(accelfd, printbuf,
					 sizeof(printbuf) - 1);
			if (res < 0) {
				if (res == -1) {
					fverbf(stderr,
		...
		if (cmdfd >= 0 && FD_ISSET(cmdfd, &rfds)) {
			...
			res = bng_read(cmdfd, transbuf + bytes_rd,
...
static int bng_connect(struct host_params *peer, bool numeric)
{
...
static void print_help(const char *name)
{
	...
	printf("\n\tCOMMAND is the micro-bng command line to be executed");
...
int main(int argc, char **argv)
{
	...
	while ((ochar = getopt_long(argc, argv, "f:46ni:H:p:t:P:vVh-",
		...
		if (ochar == '-')
			/* End of options, interpret the following arguments
			   as part of micro-bng's command */
			break;
	...
	rv = bng_talk(inputstream, peer.fd, mh, timeo);
