...
def micro_bngd_config(veth_pair_netns):
    print("micro_bngd_config veth_pair_netns: " + str(veth_pair_netns))
    return (
        """
    [modules]
    pppoe
...
def test_ipoe_shared_session_wo_auth(dhclient_instance, bng_cmd, veth_pair_netns):

    # test that dhclient (with micro-bngd) started successfully
    ...
    print("test_ipoe_shared_session_wo_auth: last bng-cmd out: " + out)
