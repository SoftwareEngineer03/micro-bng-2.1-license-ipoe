...
Then build micro-bng in 'build' directory (as usual)

Install micro-bngd (make install or use distro package). Do not run micro-bngd using systemd or other supervisors
```bash
...
# cpack -G DEB && dpkg -i micro-bng.deb
```
...
gcovr --config=tests/gcovr.conf --html --html-details --output=tests/report/micro-bng.html # html reports (most useful)
```
